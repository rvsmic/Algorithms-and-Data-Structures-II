...
inline void reset_connected(vector <Pawn*> &pawns) {
  for(int i=0;i<pawns.size();++i) {
    pawns[i]->connected = false;
    for(int j=0;j<pawns[i]->hits.size();++j) {
      pawns[i]->hits[j]->white->connected = false;
    }
  }
}
  ...
  while(!pawn_queue.empty()) {
    ...
    for(int i=0;i<current->hits.size();++i) {
      ...
      if(current->hits[i]->free_edge == on_black) {
        ...
        if(!found_match && !current_opposite->connected) {
          current_opposite->connected = true;
          cout<<current->hits[i]->black->id<<"=="<<current->hits[i]->white->id<<endl;
          current->hits[i]->free_edge = false;
          found_match = true;
        }
...
bool dfs(Pawn* current, int current_height, int &height, vector <Edge*> &temp_anwser) {
  ...
  for(int i=0;i<current->hits.size();++i) {
    ...
    if(current->hits[i]->free_edge == !on_white && current_opposite->free && current_height < height) {
      current_opposite->free = false;
      temp_anwser.push_back(current->hits[i]);
      return dfs(current_opposite,current_height + 1,height,temp_anwser);
...
int hopkroft_karp(vector <Pawn*> &pawns,int N) {
  ...
  while(true) {
    ...
    while(!pawn_queue.empty()) {
      ...
      cout<<current->id<<": ";
      pawn_queue.pop();
      if(dfs(current,0,height,temp_anwser)) {
