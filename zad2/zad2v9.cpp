...
struct Pawn {
  ...
  void add_white_edge(Pawn* opposite) {
    ...
    for(int i=0;i<hits.size();++i) {
      if(hits[i]->black->id == opposite->id) {
        already_added = true;
        break;
      }
    }
  ...
  void add_black_edge(Pawn* opposite) {
    bool already_added = false;
    for(int i=0;i<hits.size();++i) {
      if(hits[i]->white->id == opposite->id) {
        already_added = true;
        break;
      }
    }
...
bool bfs(vector <vector <Pawn*> > &pawns, vector <Pawn*> &black_pawns, queue <Pawn*> &pawn_queue, int &height, int size) {
  ...
  while(!queue.empty()) {
    ...
    for(int i=0;i<current->hits.size();++i) {
      ...
      cout<<opposite->id<<" ";
      cout<<"{"<<current->hits[i]->free_edge<<"+"<<on_black<<"} ";
      if(current->hits[i]->free_edge == on_black) {
        if(!visited[opposite->id]) {
          ...
          queue.push(opposite);
          ++future_level_cap;
          cout<<"F: "<<opposite->id<<"~"<<opposite->free<<" ";
  ...
  for(int i=0;i<current->hits.size();++i) {
    ...
    if(current->hits[i]->free_edge == on_white) {
      ...
      if(dfs(opposite,height,current_height + 1,temp_anwser) ) {
        ...
        // opposite->hits[current->id]->free_edge = !opposite->hits[current->id]->free_edge;
      ...
      if(found_anwser && on_white) {
        ...
        // opposite->free = false;
        current->hits[i]->free_edge = !current->hits[i]->free_edge;
        cout<<current->id<<" -> "<<opposite->id<<endl;
...
int hopcroft_karp(vector <vector <Pawn*> > &pawns, vector <Pawn*> &black_pawns, int size) {
  ...
  for(int i=0;i<black_pawns.size();++i) {
    cout<<black_pawns[i]->id<<":";
    for(int j=0;j<black_pawns[i]->hits.size();++j) {
      cout<<black_pawns[i]->hits[j]->white->id<<" ";
    }
    cout<<endl;
  }
  ...
  while(true) {
    ...
    cout<<endl<<"Q: "<<pawn_queue.size()<<endl;
    cout<<"DFS:\n";
    ...
    cout<<"TEMP:"<<endl;
    for(int i=0;i<temp_anwser.size();++i) {
      cout<<temp_anwser[i]->black->id<<"-"<<temp_anwser[i]->white->id<<" ";
    }
  ...
  cout<<endl<<"FINAL:"<<endl;
    for(int i=0;i<final_anwser.size();++i) {
      cout<<final_anwser[i]->black->id<<"-"<<final_anwser[i]->white->id<<" ";
    }
...
int main() {
  ...
  for(int i=0;i<N;++i) {
    for(int j=0;j<N;++j) {
      cin>>space;
      pawns[i].push_back(new Pawn(counter,i,j,space));
      ++counter;
    }
  }
