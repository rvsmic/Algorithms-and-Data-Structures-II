...
struct Red_Black_Tree {
  ...
  void fix_tree(Tree_Node* &node) {
    //jezeli rodzic jest czarny to ok
    if(node->parent->color == black) {
      return;
    
    //jak nie to sprawdzamy przypadki
    } else {
      ...
      if(grandparent != nullptr) {
        ...
        if(uncle->color == black && node == parent->right && parent == grandparent->left) {
        ...
        } else if (uncle->color == black && node == parent->left && parent == grandparent->right) {
        ...
        if(uncle->color == black && current_node == parent->left && parent == grandparent->left) {
        ...
        } else if (uncle->color == black && current_node == parent->right && parent == grandparent->right) {
...
int main() {
  ...
  for(int i=0;i<m;++i) {
    ...
    if(op == 'a') {
      ...
      cin>>s>>x;
      // List_Node* moving_node = name_map[s];
      ...
      string s1,s2;
      cin>>s1>>s2;
      // List_Node* a = name_map[s1];
      // List_Node* b = name_map[s2];
      List_Node* a = tree.get(s1);
      List_Node* b = tree.get(s2);
      people.swap_values(a,b);
      // List_Node* tmp = name_map[s1];
